 ...
 * @constructor
 */
function ColorCutQuantizerVbox(parent, lowerIndex, upperIndex) {
    this._parent = parent;
...
ColorCutQuantizerVbox.prototype.midPoint = function(dimension) {
    switch (dimension) {
        case -3:
        default:
            return Math.floor((this._minRed + this._maxRed) / 2);

        case -2:
            return Math.floor((this._minGreen + this._maxGreen) / 2);

        case -1:
            return Math.floor((this._minBlue + this._maxBlue) / 2);
    }
};

export default ColorCutQuantizerVbox;
